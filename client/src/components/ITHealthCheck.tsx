...
const API_URL = "https://api.marcoby.net/techCheck";
...
const ITHealthCheck: React.FC<ITHealthCheckProps> = ({ inlineButton }) => {
  ...
  const modalContent = (
    <div
      className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-green-200 dark:border-green-900 p-0 animate-fade-in w-full max-w-lg flex flex-col"
      style={{ maxWidth: '95vw' }}
    >
      <div className="flex justify-between items-center px-4 md:px-6 pt-4 pb-2 border-b border-gray-200 dark:border-gray-800">
        <h2 className="text-lg md:text-xl font-bold text-green-700 dark:text-green-400">Business IT Health Check</h2>
      ...
      <div className="p-4 md:p-6 flex-1 overflow-y-auto">
        {result ? (
          <div className="text-center">
            <h3 className="text-2xl font-extrabold mb-4 text-green-700 dark:text-green-400">Your IT Health Check Results</h3>
            <p className="mb-2 font-semibold text-lg">IT Health Score: <span className="text-green-700 dark:text-green-400">{result.score ?? "—"}/100</span></p>
            ...
            <a
              ...
              className="inline-block bg-green-700 text-white px-6 py-3 rounded-full font-semibold shadow hover:bg-green-800 transition text-lg mt-4"
            >
          ...
          <>
            ...
            <div className="mb-4">
              <p className="mb-2 text-lg font-medium">{QUESTIONS[step].question}</p>
              <p className="mb-4 text-sm text-gray-500 dark:text-gray-400 italic">{QUESTIONS[step].hint}</p>
              <div className="flex flex-col gap-2">
                {QUESTIONS[step].options.map((option) => (
                  <button
                    key={option}
                    className={`px-4 py-2 rounded-full border text-lg font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 ${
                      answers[QUESTIONS[step].id] === option
                        ? "bg-green-700 text-white border-green-700 shadow"
                        : "bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 hover:bg-green-50 dark:hover:bg-green-900"
                    }`}
            ...
            <div className="flex justify-between mt-4">
              <button
                onClick={handleBack}
                disabled={step === 0}
                className="px-4 py-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 disabled:opacity-50 font-medium"
              >
                Back
              </button>
              <button
                onClick={handleNext}
                className="px-4 py-2 rounded-full bg-green-700 text-white hover:bg-green-800 font-medium"
              >
                Next
              </button>
            </div>
          ...
          <>
            <div className="mb-4">
              ...
              <input
                type="text"
                className="w-full px-4 py-2 border rounded-full mb-2 text-lg focus:outline-none focus:ring-2 focus:ring-green-400"
            ...
            <div className="mb-4">
              <label className="block text-lg font-medium mb-1">Business Email</label>
              <input
                type="email"
                className="w-full px-4 py-2 border rounded-full mb-2 text-lg focus:outline-none focus:ring-2 focus:ring-green-400"
            ...
            <button
              ...
              disabled={submitting}
              className="w-full px-4 py-2 rounded-full bg-green-700 text-white hover:bg-green-800 font-semibold text-lg mt-2"
            >
    ...
    <>
      {showButton && !showModal && (
        <button
          className={
            inlineButton
              ? "bg-green-700 text-white px-6 py-3 rounded-full shadow-lg font-semibold text-lg hover:bg-green-800 transition"
              : "fixed z-40 bottom-4 right-4 left-4 md:left-auto md:bottom-8 md:right-8 bg-green-700 text-white px-4 py-3 md:px-6 rounded-full shadow-lg font-semibold text-base md:text-lg hover:bg-green-800 transition animate-bounce w-auto md:w-auto"
          }
